...
if (isset($_POST['simpan'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($nextFoto, $ext)) {
            ...
            $insert = mysqli_query($koneksi,  "INSERT INTO konten (nama_user, keterangan, foto) VALUES ('$nama_user', '$keterangan' ,'$nama_foto')");
        }
    } else {
        $insert = mysqli_query($koneksi,  "INSERT INTO konten (nama_user, keterangan) VALUES ('$nama_user', '$keterangan')");
    }


    header("location:konten.php?tambah=berhasil");
}

$id = isset($_GET['edit']) ? $_GET['edit'] : '';
$queryEdit = mysqli_query($koneksi, "SELECT * FROM konten WHERE id ='$id'");
...
if (isset($_POST['edit'])) {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array($extFoto, $ext)) {
            ...
            $update = mysqli_query($koneksi, "UPDATE konten SET nama_user='$nama_user', keterangan='$keterangan', foto='$nama_foto' WHERE id='$id'");
        }
    } else {
        // kalo user tidak ingin memasukkan gambar
        $update = mysqli_query($koneksi, "UPDATE konten SET nama_user='$nama_user', keterangan='$keterangan' WHERE id='$id'");
    }

    header("location:konten.php?ubah=berhasil");
}
?>
                                            ...
                                            <div class="mb-3 row">
                                                <div class="col-sm-6">
                                                    ...
                                                    <input type="text" class="form-control" name="nama_user" placeholder="Masukan nama anda"
                                                        required
                                                        value="<?php echo isset($_GET['edit']) ? $rowEdit['nama_user'] : '' ?>">
                                                </div>
                                                <div class="col-sm-6">
